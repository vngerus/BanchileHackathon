...
function AdminPac() {
  ...
  const [url, setUrl] = useState(`http://localhost:8080/filtrar`);
  ...
  const calculateTotalAmount = () => {
    ...
    if (selectAll) {
      ...
      totalAmount = selectedData.reduce(
        (total, data) => total + parseFloat(data[4]),
        0
      );
    }
  ...
  useEffect(() => {
    const selectedData = page.filter(
      (data, index) => selectAll || selectedRows.includes(index)
    );
  ...
  useEffect(() => {
    ...
    const atLeastOneSelected = selectedRows.length > 0 || selectAll;

    // Actualizar el estado del botón
    setIsBotonDisabled(!atLeastOneSelected);
  ...
  return (
    <>
      <div className="admin-pac-container">
        ...
        <div className="caja-blanca">
          ...
          {filtroVisible && (
            <div className="filtro-popup">
              <button className="cierre" onClick={cerrarFiltro}>
                x
              </button>
        ...
        <button className="boton-exportar" onClick={exportSelectedData} disabled={isBotonDisabled}>
